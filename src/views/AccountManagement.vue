    ...
    <div class="page-content">
      <el-card class="content-card">
        ...
        >
          ...
          <el-table-column prop="name" label="管理员姓名" min-width="120" align="center">
          ...
          <el-table-column prop="level" label="权限等级" min-width="100" align="center">
            <template #default="{ row }">
              <el-tag 
...
import { BASE_URL, adminApi } from '../config/api.js'
...
const fetchAdminList = async () => {
  loading.value = true
  try {
    const result = await adminApi.getAdmins(currentPage.value, pageSize.value)
    if (result.success) {
...
const openEditDialog = (admin) => {
  editForm.admin_id = admin.admin_id
  editForm.name = admin.name
  editForm.username = admin.username
  editForm.level = admin.level
...
const saveEdit = async () => {
  try {
    ...
    const updateData = {
      name: editForm.name,
      member_type: editForm.level === 'super' ? 'svip' : 'vip' // 映射权限等级到会员类型
    }
    
    const result = await adminApi.updateAdmin(editForm.admin_id, updateData)
...
const deleteAdmin = async (admin) => {
  try {
    const result = await adminApi.deleteAdmin(admin.admin_id)
